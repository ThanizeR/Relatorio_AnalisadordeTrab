...
def generate_pdf_aplicacao(df_aplicacao, figures, background_image_first_page_aplicacao=None, background_image_other_pages=None):
    ...
    # Lista dos gráficos que DEVEM manter legenda
    graficos_com_legenda = ['fig_taxa_aplicacao']
    ...
    try:
        ...
        combustivel_fmt = f"{combustivel_total:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")

        c.setFont("Helvetica", 8)
        c.drawString(x_margin, page_height - 40, f"Área Total Aplicada: {area_total_fmt} ha")
        c.drawString(x_margin, page_height - 60, f"Combustível Total: {combustivel_fmt} L")
        c.drawString(x_margin, page_height - 80, f"Total de Equipamentos: {total_equip}")

    except Exception as e:
        print(f"Erro ao calcular ou formatar dados: {e}")
    ...
    for position_and_size in graph_positions_and_sizes:
        ...
        grafico_nome = position_and_size['name']
        ...
        for ax in fig.get_axes():
            ...
            # 🔥 REMOVER legenda APENAS se o gráfico NÃO está na lista
            legend = ax.get_legend()
            if legend and grafico_nome not in graficos_com_legenda:
                legend.remove()
